...
class JuicerSpider(Spider):
		...
		def initialize_default_variables(self):
			...
			self.source = self.json_file = None
			self.best_sellers_file = self.products_file = self.reviews_file = None
			self.custom_query_file = None;self.created_file = None
		...
		def get_best_sellers_file(self):
			if self.best_sellers_file: return self.best_sellers_file

			best_sellers_queries_filename = os.path.join(QUERY_FILES_DIR, "%s_best_sellers_%s.queries" % (self.name, get_current_ts_with_ms()))
			self_best_sellers_file = open(best_sellers_queries_filename, 'w')

			return self._best_sellers_file
		...
		def close_all_opened_query_files(self):
			files_list = [
				self.best_sellers_file , self.products_file , self.reviews_file, self.created_file
			]
			for f in files_list:
