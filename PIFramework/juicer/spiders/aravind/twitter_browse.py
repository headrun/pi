...
import sys
sys.path.append('/home/headrun/GenFramework/juicer')
...
def parse(self, response):
	with open('twitter.txt', 'r') as f:
		...
		for row in rows:
			...
			extra_arguments = _oauth_request_parameters(TW_URL, consumer_token, access_token, arguments)
			arguments.update(extra_arguments)
			arguments = urlencode(arguments)
			url = '%s?%s' %(TW_URL, arguments)
			import pdb;pdb.set_trace()
			self.utils.get_page('twitter_profiles_terminal', url, screen_name)
