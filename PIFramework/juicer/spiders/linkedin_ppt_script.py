...
class Login(object):
    ...
    def __init__(self, options):
        ...
        self.select_qry1 = 'select exp_title, exp_company_name, start_date, end_date, exp_location from linkedin_experiences \
        where profile_sk ="%s" limit 3'
        self.select_qry2 = 'select image_path from linkedin_meta where member_id = "%s" limit 1'
        self.select_qry3 = 'select edu_school_name, edu_degree, edu_start_year, edu_end_year from linkedin_educations \
        where profile_sk="%s" limit 3'
	...
	for ind, colu in enumerate(exp_data):
		...
		try: table2.cell(0, ind+1).text = ans
                except: break
    ...
    def insert_image(self, slide, shapes, img_path, member_id):
        ...
        try: pic = slide.shapes.add_picture(img_path,\
              ...
              pic = slide.shapes.add_picture('/root/test/ppt/emptylinkedin.png',\
              left=Inches(0), top=Inches(1), width=Inches(2), height=Inches(1.5))
              print "No image Found for %s" % member_id
        #Inserting logo
