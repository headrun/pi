...
class Complaintboardreviews(JuicerSpider):
    ...
    def parse(self, response):
        ...
        for nd in nodes:
            ...
            aux_info.update({"location":location})
            if post_text: aux_info.update({"post_title":post_text})
            if author_url:
    ...
    def parse_next(self, response):
        ...
        aux_info.update({"author_url":response.url})
        ...
        if author_location: aux_info.update({"author_location":author_location})
        if author_name: aux_info.update({"author_name":author_name})
        aux_info.update({"author_url":response.url})
