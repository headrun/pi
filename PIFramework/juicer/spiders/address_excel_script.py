...
class ExcelGenIOC():
    ...
    def __init__(self):
        ...
        self.excel_file_name = 'address_dataon_20171101.xls' 
    ...
    def excel_generation(self):
                query = 'select address_original,formatted_address, geometry_bounds_ne_lat, geometry_bounds_ne_lng, geometry_bounds_sw_lat, geometry_bounds_sw_lng, location_lat, location_lng, location_type, viewport_ne_lat, viewport_ne_lng, viewport_sw_lat, viewport_sw_lng, partial_match, place_id, types, status, reference_url,long_name, short_name, component_type, mobile_no from new_address order by mobile_no '
           
                self.cur.execute(query)
                rows = self.cur.fetchall()
             
                header = ['S_no','Address','formatted_address','geometry_bounds_ne_lat','geometry_bounds_ne_lng','geometry_bounds_sw_lat','geometry_bounds_sw_lng','location_lat','location_lng','location_type','viewport_ne_lat','viewport_ne_lng','viewport_sw_lat','viewport_sw_lng','partial_match','place_id','types','status','reference_url','long_name','short_name','component_type']
