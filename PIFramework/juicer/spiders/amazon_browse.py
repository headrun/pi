...
class AmazonBestsellersbrowse(JuicerSpider):
    ...
    def parse(self, response):
        ...
        navigation_list = get_nodes(sel,'//ul[contains(@id,"browse")]//ul[li]/li/a')#[10:11]
    ...
    def parse_nexttab(self, response):
        ...
        if not nexp:
            ...
            #another_nav = another_nav[6:7]+another_nav[2:3]+another_nav[0:1]
            for av in another_nav:
                main_url = extract_data(av,'./@href')
                main_category = extract_data(av, './text()')
                if 'http' in main_url: yield Request(main_url, callback=self.parse_nexttab, meta={"main_category":main_category,"next_page":''})
        if fortext.encode('utf8'):#'Contact Lenses' in fortext.encode('utf8'):
