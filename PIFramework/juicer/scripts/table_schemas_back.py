...
episode_table = """
    CREATE TABLE `Episode` (
      ...
      `modified_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      `last_seen` datetime NOT NULL,
      PRIMARY KEY (`sk`,`season_sk`,`tvshow_sk`),
      KEY `episode_sk` (`sk`),
...
rating_table = """
    CREATE TABLE `Rating` (
      ...
      `rating_reason` varchar(255) COLLATE utf8_unicode_ci DEFAULT '',
      `created_at` datetime NOT NULL,
      `modified_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      `last_seen` datetime DEFAULT NULL,
       PRIMARY KEY (`program_sk`,`program_type`,`rating_type`),
       KEY `rating_type` (`rating_type`),
       KEY `rating_program_sk_type` (`program_sk`,`program_type`)
    )ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
"""
