...
class Voyagerapi(Voyagerapiitems):
	...
	def update_status(self, sk, crawl_status, where_status):
		delete_query = 'DELETE FROM linkedin_crawl WHERE crawl_status=%s AND sk ="%s"' % (crawl_status, sk)
		execute_query(self.cur, delete_query)
		bkup_query = 'select sk from linkedin_crawl where sk = "%s" group by sk  having count(sk)>1' % (sk)
		#update_query = 'UPDATE linkedin_crawl SET crawl_status=%s, modified_at=NOW() WHERE sk = "%s"' % (crawl_status, sk )
		update_query = 'UPDATE linkedin_crawl SET crawl_status=%s WHERE sk = "%s"' % (crawl_status, sk)
		if where_status == 0:
			#update_query = 'UPDATE linkedin_crawl SET crawl_status=%s, modified_at=NOW() WHERE sk = "%s" and crawl_status= %s' % (crawl_status, sk, where_status)
			update_query = 'UPDATE linkedin_crawl SET crawl_status=%s WHERE sk = "%s" and crawl_status= %s' % (crawl_status, sk, where_status)
		else:
			#update_query = 'UPDATE linkedin_crawl SET crawl_status=%s, modified_at=NOW() WHERE sk = "%s" and (crawl_status=9 or crawl_status=10)'%(crawl_status, sk)
			update_query = 'UPDATE linkedin_crawl SET crawl_status=%s WHERE sk = "%s" and (crawl_status=9 or crawl_status=10)'%(crawl_status, sk)
		if update_query:
			try: self.cur.execute(update_query)
			except:
				try:
					...
					if recs_:
						...
						del_qu = "delete from linkedin_crawl where sk ='%s' and modified_at not like '%s'" % (sk, str(recs_1[0][0]))
        ...
        def checking_for_limit(self, account_mail, logind_date, sk_login_self, command_prxy):
                ...
                if count_from_ and count_from_[0][0] < 9000:
                        ...
                        count_from_1 = fetchall(self.cur, "select sk, count from linkedin_loginlimit where count < 9000 and login_date='%s' and sk != '%s' order by rand() limit 1" % (logind_date, sk_login_self))
                        if count_from_1:
	...
	def alert_mail(self, sk, crawl_status, error):
		...
		receivers_mail_list = ['kiranmayi@headrun.net', 'anushab@headrun.net']
	...
	def type_of_item(self, data_elements, url_type, main_member_id, sk):
		...
		if url_type == 'groups':
			...
			item = self.get_posts_data(data_elements, url_type, main_member_id, sk)
	...
	def get_testscores_data(self, data_elements, url_type, sk):
        ...
        def get_influencers_data(self, data_elements, url_type, sk):
                ...
                influencer_no_of_members = data_elements.get('followingInfo',{}).get('followerCount','')
