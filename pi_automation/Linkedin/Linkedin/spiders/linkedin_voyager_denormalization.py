...
class Licsdenormalization(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for dirs in self.dirs:
            make_dir(dirs)
        self.con = MySQLdb.connect(host ='localhost', user = 'root', passwd = 'root', db = 'Test_PI', charset = 'utf8', use_unicode = True)
    ...
    def main(self):
		...
		data_avai = 'Not Available'
	    values_final = [str(sk_), str(sk), given_url,  status_url, data_avai]
		...
		calfun2 = self.colum(tabl, sk, inde)
                if 'linkedin_experiences' in tabl : 
                    calfun2 = calfun2[0:80]
	    ...
	    values_final = final + final
            self.cur.execute(insert_query_pi,values_final)
