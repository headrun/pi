...
class JuicerSpider(Spider):
        ...
        def get_doctorinfo_file(self):
            if self.doctorinfo_file : return self.doctorinfo_file
        ...
        def get_doctormeta_file(self):
            if self.doctormeta_file : return self.doctormeta_file
            doctormeta_queries_filename = os.path.join(QUERY_FILES_DIR, "%s_doctormeta_%s.queries" % (self.name, get_current_ts_with_ms()))
            self.doctormeta_file = open(doctormeta_queries_filename,'w')
            return self.doctormeta_file

        def get_doctorfeedback_file(self):
            if self.doctorfeedback_file : return self.doctorfeedback_file
            doctorfeedback_queries_filename = os.path.join(QUERY_FILES_DIR, "%s_doctorfeedback_%s.queries" % (self.name, get_current_ts_with_ms()))
            self.doctorfeedback_file = open(doctorfeedback_queries_filename,'w')
            return self.doctorfeedback_file

        def get_doctorhospital_file(self):
            if self.doctorhospital_file : return self.doctorhospital_file
            doctorhospital_queries_filename = os.path.join(QUERY_FILES_DIR, "%s_doctorhospital_%s.queries" % (self.name, get_current_ts_with_ms()))
            self.doctorhospital_file = open(doctorhospital_queries_filename,'w')
            return self.doctorhospital_file
