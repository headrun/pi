...
class Practohospitals(JuicerSpider):
    ...
    #start_urls = ['https://www.practo.com/chennai/hospital/k-p-bone-joint-centre-adambakkam']
    #start_urls = ['https://www.practo.com/chennai/hospital/apollo-speciality-hospitals-vanagaram']
    ...
    def parse(self, response):
        ...
        hospt_description = str(profile_payload.get('summary', '')).encode('utf8')
        ...
        hospital_address =  str(profile_payload.get('street_address',{})).encode('utf-8') + str(profile_payload.get('landmark',{})).encode('utf-8')
        ...
        #postal_code =  profile_payload.get('street_address',{})
        postal_code = ''
        ...
        amentities_list = []
        amenities = profile_payload.get('amenities','').keys()
        if amenities : 
            for amentie in amenities : 
                amenties =  profile_payload.get('amenities',{}).get(amentie,{}).get('name','')
                amentities_list.append(amenties)
            amenities = "<>".join(amentities_list)
    ...
    def dct_hsp_meta_info(self,response):
        ...
        for hnd in doctor_nodes:
            ...
            doc_name = str(hnd.get('doctor','').get('name',''))
