...
class CSVGenIOC():
    ...
    def __init__(self):
        self.load_dict = {}
        self.excel_file_name = 'baby_names6_%s.csv'%str(datetime.datetime.now().date())
    ...
    def get_mysql_conn(self):
        self.conn = MySQLdb.connect(db = 'Men_Names', user='root', host = 'localhost', passwd='hdrn59!', charset   = "utf8", use_unicode=False)
    ...
    def csv_generation(self):
        ...
        for index, sk1 in enumerate(sks):
            sk2 = sk1[0]
            sel_query = 'select name,gender,similar_names,meaning_name,popularity,name_usage,origin_usage,famous_names,reference_url from baby_names where date(modified_at)>="2018-02-16" and sk="%s"'%(sk2)
