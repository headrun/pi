...
class Linkedincrawlautomation(Voyagerapi):
    name = "linkedin_voyagercrawlautomation_browse"
    ...
    def parse(self, response):
        ...
        command_prxy = cvs = response.meta.get('proxy', '')\
            ...
            .replace('https://', '')
        logincsrf = ''.join(
            sel.xpath('//input[@name="loginCsrfParam"]/@value').extract())
        csrf_token = ''.join(
            sel.xpath('//input[@id="csrfToken-login"]/@value').extract())
        source_alias = ''.join(
            sel.xpath('//input[@name="sourceAlias"]/@value').extract())
        if self.profiles_list:
            logind_date = "%s%s" % (
                str(datetime.datetime.now().date()), ' 00:00:00')
            sk_login_self = self.login
            login_account = mails_dict[self.login]
            account_mail, account_password = login_account
            yes_s, skf_login_self = self.checking_for_limit(
                account_mail, logind_date, sk_login_self, command_prxy)
            if skf_login_self:
                login_account = mails_dict[skf_login_self]
                account_mail, account_password = login_account
                if account_mail and self.profiles_list:
                    return [FormRequest.from_response(
                        response, formname='login_form',
    ...
    def spider_closed(self, spider):
        ...
        # yield Request('https://www.linkedin.com/logout/',
        # callback=self.close_yield)
        ...
        for li in self.profiles_list:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        meta_data = ast.literal_eval(
                            normalize(li[2]).replace('\\', ''))
                    except:
                        pass
    ...
    def parse_correct(self, response):
        ...
        try:
            profile_data = json.loads(
                ''.join(sel.xpath('//code[contains(text(),"profile.ProfileView")]/text()').extract()))
        except:
            try:
                profile_data = json.loads(
                    sel.xpath('//code[contains(text(),"profile.ProfileView")]/text()').extract()[0])
            except:
                profile_data = {}

        meb = sel.xpath(
            '//code[contains(text(),"PatentView")][contains(text(),"objectUrn")][contains(text(),"urn:li:member:")]').extract()
        ...
        connections_count = textify(
            re.findall('connectionsCount&quot;:(\d+),', response.body))
        followers_count = textify(
            re.findall('followersCount&quot;:(\d+),', response.body))

        linke_a, linkd_javatex, view_url_construction, contact_info = {
        }, '', '',  {}
        try:
            contact_info = json.loads(
                sel.xpath('//code[@style="display: none"][contains(text(),"urn:li:fs_contactinfo")]/text()').extract()[0])
        except:
            contact_info = {}
        email_addresses, phone_number, twitter_handles, instant_message, birth_data_month,\
            birth_data_year, birth_data_day, websites, interestsview = [
                ''] * 9
        try:
            interestsview = ', '.join([inte['type']
                                      for inte in filter(None, [i if i['$type'] == 'com.linkedin.voyager.identity.profile.StandardProfileContactInterest' else '' for i in contact_info.get('included', [])])])
        except:
            interestsview = ''
        if contact_info:
            email_addresses = contact_info.get(
                'data', {}).get('emailAddress', '')
            phone_number = filter(
                None, [i if i['$type'] == 'com.linkedin.voyager.identity.profile.PhoneNumber' else '' for i in contact_info.get('included', [])])
            if phone_number:
                phone_numbers = phone_number[0].get('number', '')
                phone_number_type = phone_number[0].get('type', '')
                phone_number = (
                    "%s%s%s" % (phone_number_type, ':-', phone_numbers)).strip().strip(':-').strip()
            else:
                phone_number = ''
            twitter_handles = filter(
                None, [i if i['$type'] == 'com.linkedin.voyager.identity.shared.TwitterHandle' else ''for i in contact_info.get('included', [])])
            if twitter_handles:
                twitter_handles = twitter_handles[0].get('name', '')
            else:
                twitter_handles = ''
            im_handles = filter(
                None, [i if i['$type'] == 'com.linkedin.voyager.identity.profile.IM' else ''for i in contact_info.get('included', [])])
            if im_handles:
                im_handless = im_handles[0].get('id', '')
                im_provider = im_handles[0].get('provider', '')
                instant_message = (
                    "%s%s%s" % (im_provider, ':-', im_handless)).strip().strip(':-').strip()
            try:
                websites = filter(
                    None, [i if i['$type'] == 'com.linkedin.voyager.identity.profile.ProfileWebsite' else '' for i in contact_info.get('included', [])])[0].get('url', '')
            except:
                websites = ''
            birth_data_a = filter(
                None, [i if i['$type'] == 'com.linkedin.common.Date' else '' for i in contact_info.get('included', [])])
            if birth_data_a:
        ...
        try:
            linke_a = json.loads(sel.xpath('//code[contains(text(),"authToken")]/text()').extract()[
                                 0].replace('\\', '').replace('\n', '').strip()).get('included', '')
        except:
            try:
                cv = (sel.xpath('//code[contains(text(),"authToken")]/text()').extract()[
                      0].replace('\\', '').replace('\n', '').replace(' "', '').strip())
                linke_a = re.findall('included":(\[.*\])', cv)
                linke_a = json.loads(
                    re.sub('pdfFileName=.*(").*&authType=name', '', linke_a[0]))
            except:
                linke_a = {}
        if linke_a:
            enum_l = [i for i, j in enumerate(
                linke_a) if 'authToken' in j.get('requestUrl', '')]
            if enum_l:
                enumd = enum_l[0]
                linkd_javatex = linke_a[enumd].get('requestUrl', '')
                linkedin_auth = ''.join(
                    re.findall('authToken=(.*?)&', linke_a[enumd].get('requestUrl', '')))
                cvv = textify(re.findall('\?id=(.*?)&', linkd_javatex))
                if cvv:
                    view_url_construction = profile_view_url % (
                        cvv, linkedin_auth)
        ...
        if basic_data:
            ...
            track_item = self.get_track_item(
                sk, '', login_mail, command_prxy, '6', given_key, m_data)
            yield track_item
    ...
    def get_meta_profile(self, basic_dat):
        ...
        location_postal_code = basic_dat.get('location', {}).get(
            'basicLocation', {}).get('postalCode', '')
        location_country_code = basic_dat.get('location', {}).get(
            'basicLocation', {}).get('countryCode', '')
        version_tag = basic_dat.get('versionTag', '')
        summary = basic_dat.get('summary', '')
        address = basic_dat.get('address', '')
        background_image = basic_dat.get('backgroundImage', {}).get('image', {}).get(
            'com.linkedin.voyager.common.MediaProcessorImage', {}).get('id', '')
        if background_image:
            background_image = "%s%s" % (
                "https://media.licdn.com/media", background_image)
        ...
        if picture_info:
            picture_info = "%s%s" % (
                "https://media.licdn.com/media", picture_info)
    ...
    def get_basic_item(self, sk, first_name, entity_urn, headline, industry_name, last_name, location, location_postal_code, location_country_code, version_tag, summary, address, background_image, maiden_name, interests, phonetic_last_name, phonetic_firt_name, state, miniprofile, picture_info, birthdate, object_urn, tracking_id, public_identifier, ref_url, languages_data, connections_count, followers_count, view_url_construction, email_addresses, phone_number, twitter_handles, instant_message, birth_data_month, birth_data_year, birth_data_day, websites, interestsview):
        ...
        if public_identifier:
            linkedin_meta['profile_post_url'] = normalize(
                "https://www.linkedin.com/today/author/%s?trk=prof-sm" % public_identifier)
        linkedin_meta['summary'] = normalize(
        ...
        linkedin_meta['birth_month'] = normalize(
            str(birthdate.get('month', '')))
        ...
        linkedin_meta['location_country_code'] = normalize(
    ...
    def parse_voyage(self, response):
        ...
        if url_type == 'basic':
            ...
            if public_identifier or first_name:
                basic_main_data = self.get_basic_item(
                    sk, first_name, entity_urn,
                ...
                if basic_main_data:
                    if basic_main_data['profile_image']:
                        hashs = hashlib.sha1(
                            basic_main_data['profile_image'].encode('utf-8', 'strict')).hexdigest()
                        yield ImageItem(image_urls=[basic_main_data['profile_image']])
                        basic_main_data['image_path'] = "%s%s%s" % (
                            profile_images_path, hashs, '.jpg')
                    yield basic_main_data
                    self.update_status(sk, 1, '')
                    track_item = self.get_track_item(
                        sk, str(object_urn), login_mail, command_prxy, '1', given_key, m_data)
                    yield track_item
            if public_identifier:
            ...
            for dataloop in data_elements:
                data_retruned = self.type_of_item(
                    dataloop, url_type, main_member_id, sk)
                if data_retruned:
                    yield data_retruned
