...
class NewFacebookBrowse(BaseSpider):
    name = "new_facebook_browse"
    start_urls = ['https://www.facebook.com/login']
    handle_httpstatus_list = [404, 302, 303, 403, 500]

    def __init__(self, *args, **kwargs):
        super(NewFacebookBrowse, self).__init__(*args, **kwargs)
    ...
    def parse_redirect(self,response):
        ...
        #yield Request('https://mbasic.facebook.com/',callback=self.parse_next)
	...
	yield Request('https://mbasic.facebook.com/', callback=self.parse_close)
        import pdb;pdb.set_trace()	
        sel = Selector(response)
        noti_xpath = sel.xpath('//div//span[contains(text(),"temporarily")]//text()').extract()
        #noti_xpath = sel.xpath('//strong[contains(text(),"Notifications")]//text()').extract()       
        if noti_xpath : 
            #noti_xpath = 'your account is restricted for seeing some profiles' 
                print noti_xpath
                if 'unrecognised browser or device' in noti_xpath or 'restricted' in noti_xpath : 
                    user = constants_dict[self.login][0]
                    pwd = constants_dict[self.login][1] 
                    self.send_mail(noti_xpath,user,pwd)
