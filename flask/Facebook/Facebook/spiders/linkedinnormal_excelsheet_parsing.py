	...
	def main(self):
		...
		for _file in files_list:
			...
			for xl_ in sheet_list:
				...
				row_check = 0
				email_address = facebook_profile  = twitter_profile = linkedin_profile = 0
				...
				for row in final_indents:
				    	try: addr  = self.rep_spl(row[email_address])
					except: addr = ''
				    	try: fbpro = self.rep_spl(row[facebook_profile])
					except: fbpro = ''
					try: twpro = self.rep_spl(row[twitter_profile])
					except: twpro = ''
					if twpro:
						sheet_pa = "%s%s%s"%(addr,', ', twpro)
						file("twitter2.txt","ab+").write("%s\n" %sheet_pa)
					try: lipro = self.rep_spl(row[linkedin_profile])
					except: lipro = ''
					if not lipro: continue
					if addr == 'Email': continue
					meta_date_from_browse = {}
					meta_date_from_browse.update({"email_address":addr, "linkedin_url":lipro})
					sk = md5.md5("%s%s"%(addr,lipro)).hexdigest()
					if '/pub/' in lipro:
						urlclear = lipro
						...
						lipro = urlclear
					values = (sk, lipro, 'linkedin', 0,json.dumps(meta_date_from_browse),'linkedin', json.dumps(meta_date_from_browse))
