...
class LinkedinpremiumprofilesBrowse(scrapy.Spider):
    name = "linkedinpremiumprofilesi_browse"
	...
	self.modifiedat = kwargs.get('modified', '2017-05-22')
	#self.con, self.cur = get_mysql_connection(DB_HOST, REQ_DB_NAME, '')
	self.con, self.cur = get_mysql_connection(DB_HOST, 'LINKEDIN_NEW', '')
	#get_query_param = "select sk, url, meta_data from linkedin_crawl where crawl_status=0 and date(modified_at) >= '%s' limit 15"%(self.modifiedat)
	get_query_param = "select sk , url, meta_data  from LINKEDIN_NEW.linkedin_crawl where date(created_at) >= '2017-05-02' and sk in ( select sk from FACEBOOK.linkedin_connectionprofiles where name = '');"
	...
	elif self.login=='sruthi':
		account_mail = 'sruthikurra53@gmail.com'
		account_password = 'sruthi^123'
	elif self.login=='kir':
		account_mail = 'multishowkiran@gmail.com'
		account_password = 'kirany@dumbu'
