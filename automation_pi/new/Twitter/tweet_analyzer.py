...
con, cur = get_mysql_connection('176.9.181.34', DB_NAME_REQ, '')
print(DB_NAME_REQ)
...
def main():
    ...
    try:
        ...
        #open("twittererror","ab+").write(args_name.encode("utf-8"))
    ...
    sk = hashlib.md5(screen_url).hexdigest()
    #vals = (sk,args.name,name,description,location,tweets,following,followers,\
    ...
    vals  = (sk,args.name,name,description,location,tweets,following,followers,\
            ...
            detected_has,mentioned_user_name,no_of_RTs,ret_user,detected_domain,\
            detected_sou,detected_las,avg_tweets_per_day,\
            sk,args.name,name,description,location,tweets,following,followers,\
            likes,image,listed_count,timezone,lang,verified,screen_url,args.email,'',\
            detected_has,mentioned_user_name,no_of_RTs,ret_user,detected_domain,\
            detected_sou,detected_las,avg_tweets_per_day)
    screen_name = args.name
    cur.execute(twitter_qry,vals)
    cur.execute('delete from twitter_crawl where sk = "%s"  and date(created_at) < curdate()' % screen_name)
