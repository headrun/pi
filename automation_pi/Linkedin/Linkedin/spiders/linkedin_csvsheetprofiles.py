...
class Licsvfilepremium(object):
	...
	self.last_seen = options.last_seen
	...
	self.query2 = "select sk, url, meta_data, crawl_status from linkedin_crawl where  modified_at >= '%s'"% (self.mpi)
	if self.limit:
		self.query2 = "select sk, url, meta_data, crawl_status from linkedin_crawl where  modified_at >= '%s' limit %s" % (self.mpi, self.limit)
	...
	q1 = 'select * from %s where profile_sk="%s" and last_seen>= "%s"'%(tble, sk, self.mpi)
	self.cur.execute(q1)
	...
	fileds_list = list(fields)
	fil_list = list(chain.from_iterable(fileds_list))[2:-3]
	q6 = "select count(*)  from %s where last_seen >= '%s' group by profile_sk order by count(*) desc limit 1"%(table, self.mpi)
	self.cur.execute(q6)
	...
	q9  = 'select * from %s where profile_sk="%s" and last_seen >= "%s"'%(table, sk, self.mpi)
	self.cur.execute(q9)
	...
	q8 = 'select * from %s where sk ="%s" and last_seen >= "%s"'%(table, sk, self.mpi)
        self.cur.execute(q8)
...
if __name__ == '__main__':
	...
	parser.add_option('-s', '--last-seen', default='', help = 'last_seen')
