...
class Lidefault(object):
	...
	def main(self):
		for key, value in mails_dict.iteritems():
			...
			for ips in ips_list:
				get_sk_login = fetchall(self.cur, get_insert_count_qry % (key, logind_date, ips))
				if not get_sk_login:
					insert_count = execute_query(self.cur, (insert_count_qry % (key, account_mail, 0, logind_date, ips, key, account_mail, 0, logind_date, ips)))
