...
class LinkedinimgScript1(object):
	def __init__(self):
		...
		self.select_query1 = " select sk from linkedin_meta where image_path!= '' limit 1,10"
		...
		for index, pro_ids in enumerate(pro_id):
			...
			for data_image in data_images:
				...
				profile_image_path1,status = '',''
                                proxies = {'http://144.76.48.143:3279',
					   'http://144.76.48.144:3279',
					   'http://144.76.48.145:3279'}
				...
				if status1 == 200:
					...
					"""with open(mems_ids, 'wb+') as f:
						f.write(img_data.text.encode('utf8'))"""
                                        open('image.jpg','ab+').write('%s\n'%img_data.text.encode('utf-8'))
					#os.chdir(real_path)
