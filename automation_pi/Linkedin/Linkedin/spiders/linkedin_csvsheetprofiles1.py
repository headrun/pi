...
class Licsvfilepremium(object):
	...
	self.mpi = options.mpi.replace('#', ' ')
	self.limit = options.limit
	self.excel_file_name = options.file_name
	...
	self.size = 1000
	patternk = '%keys%'"""
	#self.query2 = "select sk, url, meta_data, crawl_status from linkedin_crawl where created_at >= '2018-02-02 07:24:17'"
	self.excel_file_name = 'linkedin_data_%s.csv'%str(datetime.datetime.now().date())	
	oupf = open(self.excel_file_name, 'ab+')
	self.todays_excel_file  = csv.writer(oupf)
	self.header_params =  []
	#self.query2 = "select sk, url, meta_data, crawl_status from linkedin_crawl where  date(modified_at) >= '2018-03-01' limit 0,500" 
	self.query2 = "select sk, url, meta_data, crawl_status from linkedin_crawl where url = 'https://www.linkedin.com/in/paul-raj-mama-thangaraj-5889b520'"
	...
	self.list_tables1 = ['linkedin_educations','linkedin_experiences','linkedin_honors']
        ...
        q0 = 'SELECT COLUMN_NAME FROM information_schema.columns where table_schema= "%s" and table_name = "%s"'%(DB_NAME_REQ, 'linkedin_meta')
	...
	q1 = 'select * from %s where profile_sk="%s" and date(last_seen)>= "2018-03-01"'%(tble, sk)
	self.cur.execute(q1)
	...
	fileds_list = list(fields)
	fil_list = list(chain.from_iterable(fileds_list))[2:-3]
	q6 = "select count(*)  from %s where date(last_seen) >= '2018-03-01' group by profile_sk order by count(*) desc limit 1"%(table)
	self.cur.execute(q6)
	...
	q9  = 'select * from %s where profile_sk="%s" and date(last_seen) >= "2018-03-01"'%(table, sk)
	self.cur.execute(q9)
	...
	q8 = 'select * from %s where sk ="%s" and date(last_seen) >= "2018-03-01"'%(table, sk)
        self.cur.execute(q8)
	...
	for inde, rec in enumerate(records):
		...
		if get_meb_records != ():sk = get_meb_records[0][0]
		else: sk=''
		...
		if rec[3] == 1:
			...
			data_avai = 'Not Available'
		if inde == 0: self.header_params.extend(['sno', 'original_url','id', 'status of url','Data Available/UnAvailable', 'email_id', 'key'])
		values_final.extend([snof, given_url, given_id, status_url, data_avai, email_id, keysf])
...
if __name__ == '__main__':
	...
	parser.add_option('-i', '--mpi', default='', help = 'mpi')
	parser.add_option('-l', '--limit', default='', help = 'limit')
	parser.add_option('-f', '--file_name', default='', help='file_name')
