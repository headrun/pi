...
class Licsvfilepremium(object):
	...
	self.excel_file_name = 'linkedin_%s_second.csv'%str(datetime.datetime.now().date())	
	oupf = open(self.excel_file_name, 'ab+')
	self.todays_excel_file  = csv.writer(oupf)
	self.header_params =  []
	self.query2 = "select sk, url, meta_data, crawl_status from linkedin_crawl where  date(modified_at) >= '2018-03-01' limit 493,1300"
	#self.query2 = "select sk, url, meta_data, crawl_status from linkedin_crawl where url = 'https://www.linkedin.com/in/bhavesh-shah-83aa107'"
