...
class Lvoyagergetlogin(scrapy.Spider):

    def __init__(self, name=None, **kwargs):
    	super(Lvoyagergetlogin, self).__init__(name, **kwargs)
	...
	account_sk, account_mail, account_password = ['']*3
	...
	if account_mail:
		...
		return [FormRequest.from_response(response, formname = 'login_form',formdata={'session_key':account_mail,'session_password':account_password,'isJsEnabled':'','source_app':'','tryCount':'','clickedSuggestion':'','signin':'Sign In','session_redirect':'','trk':'hb_signin','loginCsrfParam':logincsrf,'fromEmail':'','csrfToken':csrf_token,'sourceAlias':source_alias}, callback=self.parse, meta={'csrf_token':csrf_token, 'sk':account_sk})]
