...
class Licsdenormalization(object):
    ...
    def get_tables_file(self):
        tables_queries_filename = os.path.join(self.QUERY_FILES_CRAWLOUT_DIR, "%s_%s_%s.queries" % (
            'linkedin_pm_csv1', self.limit, self.get_current_ts_with_ms()))
    ...
    def colum(self, table, sk, inde):
        ...
        q9 = lnkd_limit_query % (table, sk, self.modified_at, v1_)
    ...
    def main(self):
        ...
        for inde, rec in enumerate(records):
            ...
            self.tables_file.write(
                '%s\n%s\n' % (insert_query_pm_csv1, tuple(values_final)))
