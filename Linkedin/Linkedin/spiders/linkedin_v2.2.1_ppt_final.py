...
class Login(object):
    ...
    def add_languages(self, slide, shapes, member_id):
        self.cur.execute(self.select_qry6 % member_id)
        lan = self.cur.fetchall()
        lan = lan[0][0]
        table6 = shapes.add_table(rows=2, cols=1, \
        left=Cm(26.10), top=Cm(12.95), width=Cm(7.00),\
        height=Cm(3.81)).table
        table6.columns[0].width = Cm(7.00)
        table6.cell(0, 0).text = 'anguages known'
        table6.cell(0, 0).text_frame.paragraphs[0].font.size = Pt(16)
        table6.cell(0, 0).text_frame.paragraphs[0].font.color.rgb = RGBColor(0, 0, 0)
        table6.cell(0, 0).text_frame.paragraphs[0].font.bold = False
        table6.cell(0, 0).text_frame.paragraphs[0].font.name = 'Gadugi'
        table6.cell(0, 0).text_frame.paragraphs[0].font.color.rgb = RGBColor(105, 105, 105)
        txBox_ = slide.shapes.add_textbox(left=Cm(25.70), top=Cm(12.82), width=Cm(1.20), height=Cm(1.75))
	...
	if lan: 
	    ...
	    if len(lan) == 1:
		  table6.cell(1, 0).text = lan[0] 
            ...
            table6.cell(1, 0).text_frame.paragraphs[0].font.name = 'Gadugi'
        ...
        self.add_connector(slide, shapes, Cm(25.90), Cm(13.80), Cm(7.13), Cm(0.00))
        image = urllib.URLopener()
        
        lan_icon = os.getcwd()+'/static_images/'+'languages.jpg'
        logo = slide.shapes.add_picture(lan_icon,
              left=Cm(31.60), top=Cm(12.80), width=Cm(0.80), height=Cm(0.80))
