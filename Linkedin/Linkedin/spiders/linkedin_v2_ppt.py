...
class Login(object):
	...
	shapes = slide.shapes
        ...
        self.add_honors(slide, shapes, sk)
    ...
    def add_tables(self, slide, shapes, exp_data):
        ...
        if exp_data[0][2]: table.cell(1, 1).text = exp_data[0][2].split('-')[0]+'-current'
    ...
    def prof_exp_table(self, shapes, slide, exp_data) : 
        ...
        table5.rows[1].height = Inches(0.7)
        table5.rows[2].height = Inches(0.7)
        table5.rows[3].height = Inches(0.7)
        table5.rows[4].height = Inches(0.7)
        table5.rows[5].height = Inches(0.7)
        table5.rows[6].height = Inches(0.7)
        table5.rows[7].height = Inches(0.7)
        table5.cell(0, 0).text = "Prior Experiences"
        table5.cell(0, 0).text_frame.paragraphs[0].font.size = Pt(13)
        ...
        for ind, colu in enumerate(exp_data):
                ...
                for indk, i in enumerate(exp_data):
                    if ind == 0:
                        ...
                        row = row+1
                    if ind == 1:
                        table5.cell(row, 1).text = str(i[2].split('-')[0]+'-'+i[3].split('-')[0]).strip('-')
    ...
    def add_summary(self, slide, shapes, summary):
        ...
        txBox_key = slide.shapes.add_textbox(left=Inches(0.1), \
        top=Cm(11.40), width=Inches(2.5), height=Inches(0.5))
    ...
    def add_title(self, slide, shapes, title, org, sk, location, exp_data):
        #Adding Title
        txBox = slide.shapes.add_textbox(left=Inches(3), top=Cm(0.30), width=Inches(4), height=Cm(1))
        tf = txBox.text_frame
        tf.auto_size = MSO_AUTO_SIZE.TEXT_TO_FIT_SHAPE
        tf.text = title
        tf.word_wrap = True
        ...
        tf.paragraphs[0].font.bold = True
        txBox2 = slide.shapes.add_textbox(left=Inches(3), top=Cm(1.2), width=Inches(2.8), height=Cm(1))
