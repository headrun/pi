...
insert_count_qry = "insert into linkedin_loginlimit(sk, login_mail_id, count, login_date, created_at, modified_at, last_seen) values ('%s', '%s', %s, '%s',  now(), now(), now()) ON DUPLICATE KEY UPDATE last_seen=now(), sk='%s', login_mail_id='%s', count=%s, login_date='%s'"
get_insert_count_qry = 'select count from linkedin_loginlimit where sk="%s" and login_date="%s"'
